  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userStatus === 'PENDING') {
        // PENDING teacher — should not normally reach here (API blocks it),
        // but handle gracefully just in case
        setError('Il tuo account è in attesa di approvazione.');
        return;
      }
      setCurrentView(
        role === 'ADMIN'
      ...
      >
        ...
        <Card className="border-amber-200/50 shadow-xl bg-white/80 backdrop-blur-sm">
          <CardHeader className="text-center pb-2">
            <CardTitle className="text-xl text-amber-900">Accedi</CardTitle>
